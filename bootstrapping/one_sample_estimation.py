...
def Simulate_main(n, r_0, d_0, lambda_0, r_1, d_1, lambda_1, mu , alpha):
    ...
    for i in range(len(Clones_Growth)):
        ...
        t = starting_time
        current_size = 1
        total_pop = n * math.exp(lambda_0 * t) + 1 / (lambda_1 - lambda_0) * pow(n, 1-alpha) * math.exp(lambda_1 * t) * (
                1 - math.exp((lambda_0 - lambda_1) * t))
        ...
        while t < T_end:
            ...
            if current_size == 0:
                break
            total_pop = n * math.exp(lambda_0 * t) + 1 / (lambda_1 - lambda_0) * pow(n, 1 - alpha) * math.exp(
                lambda_1 * t) * (1 - math.exp((lambda_0 - lambda_1) * t))
            r1 = r_1 * (1 - total_pop / C)
            v1 = np.random.rand(1)
            next_birth = - 1 / (r1 * current_size) * math.log(v1)
            v2 = np.random.rand(1)
